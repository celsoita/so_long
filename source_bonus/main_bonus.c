...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/11 15:17:14 by cschiavo          #+#    #+#             */
/*   Updated: 2023/01/11 15:17:17 by cschiavo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	if (ft_mat_reader("maps/list_maps.txt", &m.maps) != -1)
	{
		if (ft_mat_reader("source_bonus/README.txt", &m.info) != -1)
		{
			m.opt.x = 0;
			m.opt.y = 0;
			m.menu.mlx = mlx_init();
			...
			m.img2 = mlx_xpm_file_to_image(m.menu.mlx, "textures/z_info.xpm",
					&m.size.x, &m.size.y);
			m.menu.win = ft_new_window(&m.menu, m.size.x, m.size.y, "menu");
